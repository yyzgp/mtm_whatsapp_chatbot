...
<aside
    ...
    class="fixed inset-y-0 left-0 z-30 w-64 bg-gray-900 text-white transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 lg:flex lg:flex-col"
>
    <!-- Logo -->
    <div class="flex h-16 items-center px-6 bg-gray-800 border-b border-gray-700">
        <div class="flex items-center gap-3">
            ...
            <span class="text-lg font-bold text-white">MTM CRM</span>
        </div>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        @canany(['customers.view', 'customers.view_all'])
        ...
        @canany(['chat.view', 'chat.view_all'])
        <a href="{{ route('chat.index') }}"
            ...
            Chat
            @php
                $unreadCount = auth()->check() ? \App\Models\ChatConversation::where('unread_count', '>', 0)->count() : 0;
            @endphp
            @if($unreadCount > 0)
        ...
        <a href="{{ route('workflows.index') }}"
            class="nav-item {{ request()->routeIs('workflows.*') ? 'active' : '' }}">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
